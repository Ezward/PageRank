 ...
 */
public class SparseArrayVector<K extends Object> implements SparseVector<K>
{
    private int count;              // the number of set elements (non zero elements)
    private final int[] indices;    // the vector indices
    private final Object[] values;  // the corresponding element value
    public final K zero;            // the default value for elements that are not set.
     ...
     */
    public SparseArrayVector(final int theCardinality)
    {
        this(theCardinality, null);
    }
     ...
     */
    public SparseArrayVector(final int theCardinality, final K zero)
    {
        this.count = 0;
        this.zero = zero;
        this.indices = new int[theCardinality];
        this.values = new Object[theCardinality];
    }
    ...
    public K get(final int theVectorIndex, final K theDefault)
    {
        ...
        if ((theArrayPosition < count) && (indices[theArrayPosition] == theVectorIndex))
        {
            return (K)values[theArrayPosition];
        }
     ...
     */
    private boolean insertAt(final int theArrayPosition, final int theVectorIndex, final K theValue)
    {
